...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t_step in range(T):
        prev_h, cache_temp = rnn_step_forward(x[:,t_step,:],prev_h,Wx,Wh,b)
        h[:,t_step,:] = prev_h
...
def rnn_backward(dh, cache):
    ...
    for t_step in reversed(range(T)):
        curr_dh = dprev_h + dh[:,t_step,:]
        dx[:,t_step,:], dprev_h, dWx_temp, dWh_temp, db_temp = rnn_step_backward(curr_dh, cache[t_step])
        dWx = dWx + dWx_temp
        dWh = dWh + dWh_temp
        db = db + db_temp
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dtanh_ct = dnext_h*o
    dnext_c += dtanh_ct*(1-np.tanh(next_c)**2)
    df = dnext_c*prev_c
    dprev_c = dnext_c*f
    di = dnext_c*g
    dg = dnext_c*i
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t_step in range(T):
        prev_h, prev_c, cache_temp = lstm_step_forward(x[:,t_step,:], prev_h, prev_c, Wx, Wh, b)
        h[:,t_step,:] = prev_h
        cache.append(cache_temp)
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def lstm_backward(dh, cache):
    ...
    for t_step in reversed(range(T)):
        curr_dh = dprev_h + dh[:,t_step,:]
        dx[:,t_step,:], dprev_h, dprev_c, dWx_temp, dWh_temp, db_temp = lstm_step_backward(curr_dh, dprev_c, cache[t_step])
        dWx = dWx + dWx_temp
        dWh = dWh + dWh_temp
        db = db + db_temp
